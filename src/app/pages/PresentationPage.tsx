...
const SCREENS = [
  { id: 1, label: "Tableau de Bord", sublabel: "Vue d'ensemble & Orientation", icon: <LayoutDashboard size={11} />, color: C.accent, Component: S1 },
  { id: 2, label: "Marchés", sublabel: "BRVM · Obligations · Change", icon: <TrendingUp size={11} />, color: C.green, Component: S2 },
  { id: 3, label: "Intelligence Macro", sublabel: "UEMOA · BCEAO · Souverain", icon: <Globe2 size={11} />, color: C.gold, Component: S3 },
  { id: 4, label: "Analyse Financière", sublabel: "Sociétés cotées · Risque", icon: <BarChart3 size={11} />, color: C.purple, Component: S4 },
...
function ScreenFallback({ label }: { label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", background: C.dark, color: C.dim, fontSize: 12 }}>
      Chargement de {label}…
    </div>
  );
}
    ...
    <div style={{ width: "100vw", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", background: C.dark, fontFamily: "'Inter', 'SF Pro Text', system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{ height: 40, flexShrink: 0, background: C.surface, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", padding: "0 12px", gap: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, paddingRight: 12, borderRight: `1px solid ${C.border}`, flexShrink: 0 }}>
          <div style={{ width: 22, height: 22, borderRadius: 5, background: "linear-gradient(135deg, #d6b68d 0%, #d6b68d 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ color: "#fff", fontSize: 8.5, fontWeight: 800 }}>BT</span>
          </div>
          <div>
            <span style={{ fontSize: 10, fontWeight: 800, color: C.text }}>BLOOMFIELD TERMINAL</span>
            <span style={{ fontSize: 8, color: C.muted, marginLeft: 6 }}>Présentation Produit · v3.0</span>
          </div>
          <span style={{ marginLeft: 6, fontSize: 7.5, padding: "1px 6px", borderRadius: 2, background: "rgba(244,185,66,0.12)", border: "1px solid rgba(244,185,66,0.25)", color: C.gold, fontWeight: 700, display: "flex", alignItems: "center", gap: 3 }}>
            ...
            return (
              <button key={s.id} onClick={() => startTransition(() => setActiveId(s.id))}
                ...
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontSize: 9, fontWeight: isActive ? 800 : 500, color: isActive ? s.color : C.muted, lineHeight: 1, whiteSpace: "nowrap" }}>
                    {s.id}. {s.label}
                  </div>
                  {isActive && <div style={{ fontSize: 7, color: s.color, opacity: 0.75, lineHeight: 1, marginTop: 1 }}>{s.sublabel}</div>}
                </div>
              </button>
            );
          })}
        ...
        <div style={{ display: "flex", gap: 4, marginLeft: 10, paddingLeft: 10, borderLeft: `1px solid ${C.border}`, flexShrink: 0 }}>
          {SCREENS.map((s) => (
            <button key={`dot-${s.id}`} onClick={() => startTransition(() => setActiveId(s.id))}
              style={{ width: s.id === activeId ? 14 : 6, height: 6, borderRadius: 3, background: s.id === activeId ? s.color : "rgba(44, 61, 127,0.4)", border: "none", cursor: "pointer", transition: "all 0.2s", padding: 0 }} />
          ))}
        </div>

        {/* Screen indicator */}
        <div style={{ marginLeft: 10, paddingLeft: 10, borderLeft: `1px solid ${C.border}`, flexShrink: 0 }}>
          <span style={{ fontSize: 8.5, color: C.muted }}>{activeIdx + 1}</span>
          <span style={{ fontSize: 8.5, color: C.muted }}>/{SCREENS.length}</span>
        </div>
      </div>
