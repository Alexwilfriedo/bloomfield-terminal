...
const INDICES = [
  { id: "brvm-c", name: "BRVM Composite", value: "284.12", change: "+2.06", pct: "+0.73%", up: true, color: C.accent, data: [281.1, 281.8, 282.5, 282.0, 283.2, 283.8, 284.0, 284.12] },
  { id: "brvm-10", name: "BRVM 10", value: "437.80", change: "+1.96", pct: "+0.45%", up: true, color: C.green, data: [435.0, 435.5, 436.1, 435.8, 436.5, 437.0, 437.4, 437.80] },
  { id: "brvm-p", name: "BRVM Prestige", value: "158.34", change: "−0.35", pct: "−0.22%", up: false, color: C.red, data: [159.1, 158.9, 158.7, 158.9, 158.6, 158.5, 158.4, 158.34] },
];
...
const SIGNALS = [
  { label: "Volume inhabituel", target: "PALM CI", color: C.gold, dot: "🔶" },
  { label: "Nouveau sommet 52S", target: "SONATEL", color: C.green, dot: "🟢" },
  { label: "Signal vendeur", target: "SOLIBRA", color: C.red, dot: "🔴" },
];
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div key={idx.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", background: "rgba(0, 1, 23,0.45)", borderRadius: 6, border: `1px solid ${C.border}` }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 5, marginTop: 1 }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: C.text, fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em" }}>{idx.value}</span>
                <span style={{ fontSize: 10, fontWeight: 600, color: idx.up ? C.green : C.red }}>{idx.pct}</span>
                <span style={{ fontSize: 9, color: idx.up ? C.green : C.red, opacity: 0.7 }}>({idx.change})</span>
              </div>
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {TOP_MOVERS.map((m) => (
              <div key={m.ticker} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 6px", borderRadius: 4, background: "rgba(0, 1, 23,0.3)", cursor: "pointer" }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: C.text, flex: 1 }}>{m.ticker}</span>
                <span style={{ fontSize: 9, color: C.dim, fontVariantNumeric: "tabular-nums" }}>{m.price}</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: m.up ? C.green : C.red, minWidth: 46, textAlign: "right" }}>{m.pct}</span>
                <span style={{ fontSize: 8, color: C.muted, minWidth: 32, textAlign: "right" }}>{m.vol}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: C.muted, letterSpacing: "0.08em", textTransform: "uppercase" }}>Signaux Intrajournaliers</span>
          </div>
