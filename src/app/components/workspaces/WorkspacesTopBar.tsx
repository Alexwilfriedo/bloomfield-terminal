    ...
    <div style={{ flexShrink: 0, background: C.surface, borderBottom: `1px solid ${C.border}`, zIndex: 50 }}>
      ...
      <div style={{ height: 52, display: "flex", alignItems: "center", padding: "0 16px", gap: 8, borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <div style={{ width: 28, height: 28, borderRadius: 6, background: "linear-gradient(135deg, #d6b68d 0%, #d6b68d 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ color: "#fff", fontSize: 11, fontWeight: 700 }}>BT</span>
          </div>
          <div>
            <div style={{ color: C.text, fontSize: 12, fontWeight: 700, lineHeight: 1, letterSpacing: "0.01em" }}>BLOOMFIELD</div>
            <div style={{ color: C.accent, fontSize: 8, fontWeight: 600, lineHeight: 1, letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 1 }}>TERMINAL</div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7, paddingLeft: 10, borderLeft: `1px solid ${C.border}`, flexShrink: 0 }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.text, letterSpacing: "0.06em", textTransform: "uppercase", lineHeight: 1 }}>WORKSPACES</div>
            <div style={{ fontSize: 8, color: C.muted, marginTop: 1, letterSpacing: "0.03em" }}>Bureau d'Intelligence Personnalisé</div>
          </div>
        </div>
        ...
        <div style={{ position: "relative", flexShrink: 0 }}>
          ...
          >
            ...
            <div style={{ textAlign: "left" }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: C.text, lineHeight: 1 }}>{current.label}</div>
              <div style={{ fontSize: 7.5, color: C.muted, marginTop: 1 }}>Workspace actif</div>
            </div>
              ...
              {WORKSPACES.map((ws) => (
                <button key={ws.id} onClick={() => { setActiveWs(ws.id); setWsOpen(false); }}
                  style={{
                    ...
                    border: "none", borderBottom: `1px solid rgba(44, 61, 127,0.16)`, cursor: "pointer",
                    color: ws.id === activeWs ? C.purple : C.dim, fontSize: 10, fontWeight: ws.id === activeWs ? 700 : 500,
                    textAlign: "left",
                  }}>
                  <span style={{ fontSize: 11 }}>{ws.icon}</span>
                  {ws.label}
                  {ws.id === activeWs && <span style={{ marginLeft: "auto", fontSize: 7.5, color: C.purple, fontWeight: 700 }}>ACTIF</span>}
                </button>
              ))}
              <button style={{ width: "100%", display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", background: "transparent", border: "none", cursor: "pointer", color: C.accent, fontSize: 10, fontWeight: 600 }}>
        ...
        <div style={{ flex: 1, maxWidth: 460, position: "relative", display: "flex", alignItems: "center" }}>
          ...
          />
          <span style={{ position: "absolute", right: 8, fontSize: 9, color: C.muted, background: "rgba(44, 61, 127,0.25)", border: `1px solid ${C.border}`, borderRadius: 4, padding: "1px 5px", fontWeight: 500, pointerEvents: "none" }}>⌘K</span>
        </div>
        ...
        <button style={{ display: "flex", alignItems: "center", gap: 7, padding: "4px 8px", borderRadius: 6, background: "rgba(0, 1, 23,0.5)", border: `1px solid ${C.border}`, cursor: "pointer", flexShrink: 0 }}>
          ...
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontSize: 8, color: C.muted, lineHeight: 1, marginTop: 2 }}>Premium · Analyste Senior</div>
          </div>
      ...
      <div style={{ height: 32, display: "flex", alignItems: "center", padding: "0 16px", gap: 10, background: "rgba(0, 1, 23,0.5)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          ...
          <span style={{ fontSize: 9, color: C.muted }}>Disposition verrouillée</span>
          <button style={{ fontSize: 8, color: C.accent, background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>Modifier</button>
        </div>
        <VDivider />
        <span style={{ fontSize: 9, color: C.muted }}>Dernière sauvegarde : 14h30</span>
        <VDivider />
        <div style={{ display: "flex", gap: 5 }}>
          {["Marchés", "BCEAO", "Obligations", "Watchlist", "Macro"].map((tag) => (
            <span key={tag} style={{ padding: "1px 6px", borderRadius: 10, background: "rgba(214, 182, 141,0.08)", border: "1px solid rgba(214, 182, 141,0.15)", fontSize: 8, color: C.accent, fontWeight: 500 }}>
              {tag}
            </span>
          ))}
        </div>
        ...
        <span style={{ fontSize: 8.5, color: C.muted, cursor: "pointer" }}>Paramètres du workspace</span>
      </div>
    </div>
  );
}
