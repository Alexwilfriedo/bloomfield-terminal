...
const RISK_SIGNALS = [
  {
    id: 1,
    type: "Risque Crédit",
    icon: <TrendingDown size={11} color={"#f4b942"} />,
    severity: "MODÉRÉ",
  ...
  },
  {
    id: 2,
    type: "Risque Taux",
    icon: <AlertTriangle size={11} color={"#fb923c"} />,
    severity: "MODÉRÉ",
  ...
  },
  {
    ...
    icon: <CheckCircle2 size={11} color={"#10c87a"} />,
    severity: "FAIBLE",
    severityColor: "#10c87a",
  ...
  },
  {
    id: 4,
    type: "Risque Marché",
    icon: <Info size={11} color={"#d6b68d"} />,
    severity: "FAIBLE",
    severityColor: "#d6b68d",
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
        ...
        {count !== undefined && (
          <span style={{ fontSize: 10, color: C.muted, background: "var(--bt-border-a20)", borderRadius: 8, padding: "0 5px" }}>{count}</span>
        )}
        {badge !== undefined && badge > 0 && (
          <span style={{ fontSize: 10, fontWeight: 700, color: "#000117", background: C.gold, borderRadius: 8, padding: "0 5px" }}>{badge}</span>
        )}
      </div>
    </div>
  );
        ...
        <div style={{ padding: "4px 0 5px" }}>
          {RISK_SIGNALS.map((signal) => (
            <div
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.background = signal.triggered ? signal.severityColor + "0c" : "var(--bt-accent-a06)")
              }
              onMouseLeave={(e) =>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 1 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: C.muted }}>{signal.type}</span>
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                ...
                <div style={{ fontSize: 11, fontWeight: 600, color: signal.triggered ? C.text : C.dim, lineHeight: 1.3 }}>
                  {signal.title}
                </div>
                <div style={{ fontSize: 9.5, color: C.muted, marginTop: 2, lineHeight: 1.3 }}>{signal.detail}</div>
          ...
          {ANALYST_NOTES.map((note) => (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: C.text }}>{note.analyst}</div>
                  <div style={{ fontSize: 9.5, color: C.muted }}>{note.role}</div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  ...
                  {note.target && (
                    <div style={{ fontSize: 9.5, color: C.muted, marginTop: 1 }}>Cible: {note.target}</div>
                  )}
                </div>
              </div>
              <div style={{ fontSize: 10.5, color: C.dim, lineHeight: 1.45 }}>{note.note}</div>
              <div style={{ fontSize: 9.5, color: C.muted, marginTop: 3 }}>{note.date}</div>
            </div>
          ))}
        </div>
      </div>
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
                <div style={{ flexShrink: 0 }}>
                  <div style={{ fontSize: 16, lineHeight: 1 }}>{pub.icon}</div>
                  <div
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 3 }}>
                    ...
                    <span style={{ fontSize: 9.5, color: C.muted }}>{pub.date}</span>
                  </div>
                </div>
