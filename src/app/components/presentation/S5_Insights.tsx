...
import { C, DataBadge, MiniWidget, BFDImpactBlock, BFDOpportunite } from "./BloomfieldSignature";
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", minHeight: 0 }}>
        ...
        <div style={{ width: 360, flexShrink: 0, borderRight: `1px solid ${C.border}`, overflow: "auto", padding: 10, display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          <div style={{ padding: "14px 14px", background: C.surface, border: `1px solid ${C.border}`, borderLeft: `3px solid ${C.accent}`, borderRadius: 7, position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, right: 0, width: 120, height: 120, background: "radial-gradient(circle, var(--bt-accent-a06) 0%, transparent 70%)", pointerEvents: "none" }} />
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 8 }}>
              <span style={{ fontSize: 9.5, padding: "2px 7px", borderRadius: 2, background: C.accent + "18", border: `1px solid ${C.accent}35`, color: C.accent, fontWeight: 800, letterSpacing: "0.06em" }}>{FEATURED.tag}</span>
              <span style={{ marginLeft: "auto", fontSize: 11, padding: "1px 6px", borderRadius: 8, background: C.accent + "14", color: C.accent, fontWeight: 700 }}>BFD Score {FEATURED.score}</span>
            </div>
            <div style={{ fontSize: 16, fontWeight: 800, color: C.text, lineHeight: 1.35, marginBottom: 8 }}>{FEATURED.title}</div>
            <p style={{ margin: "0 0 10px", fontSize: 11.5, color: C.dim, lineHeight: 1.6 }}>{FEATURED.summary}</p>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ display: "flex", gap: 1 }}>{[1,2,3,4,5].map((i) => <span key={i} style={{ fontSize: 12, color: i <= 5 ? C.gold : C.muted }}>★</span>)}</div>
              <span style={{ fontSize: 10.5, color: C.muted }}>{FEATURED.author}</span>
              <span style={{ fontSize: 10.5, color: C.muted, display: "flex", alignItems: "center", gap: 2 }}><Clock size={8} />{FEATURED.readTime}</span>
              <button style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 4, background: C.accent + "14", border: `1px solid ${C.accent}30`, color: C.accent, fontSize: 10.5, fontWeight: 700, cursor: "pointer" }}>
          ...
          <MiniWidget title="Fil Flash · BFD News" accent={C.red} topRight={<DataBadge source="BFD Newsroom" live />}>
            <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
              {FLASH_NEWS.map((n) => (
                <div key={n.id} style={{ display: "flex", gap: 6, padding: "5px 0", borderBottom: `1px solid var(--bt-border-a12)`, cursor: "pointer", alignItems: "flex-start" }}>
                  <span style={{ fontSize: 9.5, fontWeight: 700, color: C.muted, minWidth: 36, flexShrink: 0, marginTop: 1 }}>{n.time}</span>
                  <span style={{ fontSize: 9.5, padding: "1px 4px", borderRadius: 2, background: n.color + "14", color: n.color, fontWeight: 700, flexShrink: 0, alignSelf: "flex-start" }}>{n.tag}</span>
                  <span style={{ fontSize: 10.5, color: n.urgent ? C.text : C.dim, lineHeight: 1.4, fontWeight: n.urgent ? 600 : 400 }}>{n.text}</span>
                </div>
              ))}
            </div>
          </MiniWidget>
          ...
          <MiniWidget title="BFD Web TV · Dernières Diffusions" accent={C.red}>
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ width: 80, height: 52, borderRadius: 5, background: "var(--bt-accent-a10)", border: `1px solid ${C.accent}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "absolute", bottom: 3, right: 3, fontSize: 9.5, fontWeight: 700, color: "#fff", background: "rgba(0,0,0,0.7)", borderRadius: 2, padding: "0 3px" }}>12:34</div>
              </div>
              <div style={{ flex: 1 }}>
                <span style={{ fontSize: 9.5, color: C.red, fontWeight: 700 }}>WEB TV · EN DIRECT</span>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: C.text, lineHeight: 1.3, marginTop: 2 }}>Flash Marché BRVM — Session 08 Avr · Volumes & Flux institutionnels</div>
                <div style={{ fontSize: 10, color: C.muted, marginTop: 3 }}>Adjoua Koné · Analyste Marchés BFD</div>
              </div>
            </div>
          </MiniWidget>
        </div>
          ...
          {ANALYST_NOTES.map((note) => (
            <div key={note.id} style={{ padding: "11px 13px", background: C.surface, border: `1px solid ${C.border}`, borderLeft: `3px solid ${note.color}`, borderRadius: 7 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 7 }}>
                <span style={{ fontSize: 9.5, padding: "1px 5px", borderRadius: 2, background: note.color + "18", border: `1px solid ${note.color}30`, color: note.color, fontWeight: 800 }}>{note.type}</span>
                {note.rating && (
                  <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 3, background: C.green, color: "#fff", fontWeight: 800 }}>{note.rating}</span>
                )}
                {note.target && <span style={{ fontSize: 11, color: C.text }}>Cible : <strong style={{ color: C.green }}>{note.target}</strong></span>}
                <span style={{ marginLeft: "auto", fontSize: 10, color: C.muted }}>{note.date} Avr 2026</span>
              </div>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: C.text, lineHeight: 1.3, marginBottom: 6 }}>{note.title}</div>
              <div style={{ fontSize: 10, color: C.muted, marginBottom: 8 }}>{note.analyst}</div>
        ...
        <div style={{ width: 248, flexShrink: 0, borderLeft: `1px solid ${C.border}`, overflow: "auto", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>BFD Intelligence · Recommandations</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {AI_RECS.map((r) => (
                <div key={r.title} style={{ display: "flex", gap: 7, padding: "6px 8px", borderRadius: 4, background: "var(--bt-overlay-40)", border: `1px solid var(--bt-border-a20)`, cursor: "pointer" }}>
                  <div style={{ minWidth: 26, height: 20, borderRadius: 3, background: r.color + "18", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontSize: 11, fontWeight: 800, color: r.color }}>{r.score}</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", gap: 4, marginBottom: 2 }}>
                      <span style={{ fontSize: 9, padding: "0 4px", borderRadius: 2, background: r.color + "14", color: r.color, fontWeight: 700 }}>{r.tag}</span>
                    </div>
                    <div style={{ fontSize: 10.5, color: C.dim, lineHeight: 1.35 }}>{r.title}</div>
                    <div style={{ fontSize: 9.5, color: C.muted, marginTop: 2 }}>{r.reason}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>Contenu Sauvegardé</span>
            </div>
            {SAVED.map((s, i) => (
              <div key={i} style={{ display: "flex", gap: 7, padding: "5px 0", borderBottom: `1px solid var(--bt-border-a12)`, cursor: "pointer" }}>
                <span style={{ fontSize: 9.5, padding: "1px 4px", borderRadius: 2, background: s.color + "14", border: `1px solid ${s.color}25`, color: s.color, fontWeight: 800, alignSelf: "flex-start", marginTop: 1 }}>{s.tag}</span>
                <div>
                  <div style={{ fontSize: 10.5, color: C.dim, lineHeight: 1.35 }}>{s.title}</div>
                  <div style={{ fontSize: 9.5, color: C.muted, marginTop: 1 }}>Sauvegardé le {s.saved} Avr</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: "8px 10px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>Alertes Publications</span>
            </div>
            {[
              ...
              <div key={i} style={{ display: "flex", gap: 6, padding: "4px 7px", borderRadius: 3, background: a.color + "08", border: `1px solid ${a.color}18`, marginBottom: 4, cursor: "pointer" }}>
                ...
                <div>
                  <div style={{ fontSize: 10.5, color: C.dim }}>{a.msg}</div>
                  <div style={{ fontSize: 9.5, color: C.muted, marginTop: 1, display: "flex", gap: 2, alignItems: "center" }}>
                    <Clock size={7} />{a.time}
                  </div>
                </div>
              </div>
