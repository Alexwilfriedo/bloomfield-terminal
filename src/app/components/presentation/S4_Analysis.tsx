    ...
    <div style={{ display: "flex", gap: 6, alignItems: "flex-end", height: 80, padding: "0 4px" }}>
      {REVENUE_YEARS.map((r) => (
        <div key={r.y} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
          ...
          <span style={{ fontSize: 10, color: r.y === "2026P" ? C.gold : C.muted, fontWeight: r.y === "2026P" ? 700 : 400 }}>{r.y}</span>
        </div>
      ))}
    </div>
  );
        ...
        <div style={{ width: 350, flexShrink: 0, borderRight: `1px solid ${C.border}`, overflow: "auto", padding: 10, display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {COMPANIES.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => setActiveCompany(c.id)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "4px 9px", borderRadius: 5, border: `1px solid ${isActive ? rc + "50" : "var(--bt-border-a22)"}`, background: isActive ? rc + "12" : "var(--bt-overlay-50)", cursor: "pointer" }}>
                  <span style={{ fontSize: 11, fontWeight: isActive ? 700 : 500, color: isActive ? rc : C.muted }}>{c.name}</span>
                </button>
              );
            })}
          </div>
          ...
          <div style={{ padding: "12px 14px", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 7 }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
              <div style={{ width: 40, height: 40, borderRadius: 8, background: rColor + "18", border: `1px solid ${rColor}35`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: rColor, flexShrink: 0 }}>{company.name.slice(0, 2)}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: C.text }}>{company.name}</div>
                <div style={{ fontSize: 11, color: C.dim }}>{company.sector} · {company.country}</div>
                <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 }}>
                  <span style={{ fontSize: 20, fontWeight: 800, color: C.text, fontVariantNumeric: "tabular-nums" }}>{company.price} XOF</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: company.up ? C.green : C.red }}>{company.pct}</span>
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 9.5, color: C.muted }}>Cap. Boursière</div>
                <div style={{ fontSize: 14, fontWeight: 700, color: C.gold }}>{company.cap} XOF</div>
              </div>
            </div>
          ...
          <MiniWidget title="Indicateurs Financiers Clés" topRight={<DataBadge source="Rapport Annuel 2025" />}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
              {KPI.map((k) => (
                <div key={k.label} style={{ padding: "6px 8px", background: "var(--bt-overlay-50)", borderRadius: 4, border: `1px solid ${C.border}` }}>
                  <div style={{ fontSize: 10, color: C.muted, marginBottom: 2 }}>{k.label}</div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: k.color, fontVariantNumeric: "tabular-nums" }}>{k.val}</div>
                  <div style={{ fontSize: 9.5, color: k.color, opacity: 0.8 }}>{k.sub}</div>
                </div>
              ))}
            </div>
          </MiniWidget>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 10, display: "flex", flexDirection: "column", gap: 9, minWidth: 0 }}>
          ...
          <MiniWidget title="Historique Financier Pluriannuel · SONATEL (Mds XOF)" topRight={<DataBadge source="Rapports Annuels BFD" time="2021–2026P" />}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "50px repeat(6, 1fr)", gap: 0, padding: "2px 4px", borderBottom: `1px solid ${C.border}`, marginBottom: 3 }}>
              {["", ...REVENUE_YEARS.map((r) => r.y)].map((h, i) => (
                <div key={`yh-${i}`} style={{ fontSize: 9.5, fontWeight: 700, color: i === REVENUE_YEARS.length ? C.gold : C.muted, textAlign: "right", paddingRight: 4 }}>{h}</div>
              ))}
            </div>
            {[
              ...
              <div key={row.label} style={{ display: "grid", gridTemplateColumns: "50px repeat(6, 1fr)", padding: "3px 4px", borderBottom: `1px solid var(--bt-border-a12)` }}>
                <span style={{ fontSize: 10.5, color: C.muted }}>{row.label}</span>
                {REVENUE_YEARS.map((r, i) => (
                  <span key={`${row.label}-${i}`} style={{ fontSize: 11, color: r.y === "2026P" ? C.gold : C.dim, textAlign: "right", paddingRight: 4, fontVariantNumeric: "tabular-nums", fontWeight: r.y === "2026P" ? 700 : 400 }}>
                    {String(r[row.key])}
                  </span>
                ))}
              </div>
              ...
              {RATIOS_VS_SECTOR.map((r) => {
                ...
                return (
                  <div key={r.label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 10.5, color: C.muted, minWidth: 80 }}>{r.label}</span>
                    <div style={{ flex: 1, height: 6, background: "var(--bt-border-a20)", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${barW}%`, background: better ? C.green : C.red, borderRadius: 3 }} />
                    </div>
                    <span style={{ fontSize: 12, fontWeight: 700, color: better ? C.green : C.red, minWidth: 38, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{r.company}{r.unit}</span>
                    <span style={{ fontSize: 10.5, color: C.muted, minWidth: 10 }}>vs</span>
                    <span style={{ fontSize: 11, color: C.muted, minWidth: 34, textAlign: "right" }}>{r.sector}{r.unit}</span>
                  </div>
                );
              })}
            </div>
          ...
          <MiniWidget title="Benchmark Sectoriel · Télécoms UEMOA" topRight={<DataBadge source="BFD Equity" />}>
            ...
            {SECTOR_BENCHMARK.map((s) => (
              <div key={s.company} style={{ display: "grid", gridTemplateColumns: "1fr 70px 60px 55px 70px", padding: "5px 4px", borderBottom: `1px solid var(--bt-border-a12)`, alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: s.company === "SONATEL" ? C.accent : C.text }}>{s.company}</div>
                  <div style={{ fontSize: 9.5, color: C.muted }}>{s.country}</div>
                </div>
                <span style={{ fontSize: 12, color: C.dim }}>{s.marge}</span>
                <span style={{ fontSize: 12, color: C.dim }}>{s.roe}</span>
                <span style={{ fontSize: 12, color: C.dim }}>{s.pe}</span>
                <span style={{ fontSize: 10, padding: "1px 5px", borderRadius: 2, background: s.ratingColor + "14", border: `1px solid ${s.ratingColor}25`, color: s.ratingColor, fontWeight: 800, textAlign: "center" }}>{s.rating}</span>
              </div>
            ))}
          </MiniWidget>
        </div>
        ...
        <div style={{ width: 248, flexShrink: 0, borderLeft: `1px solid ${C.border}`, overflow: "auto", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>Note Analyste BFD</span>
            </div>
            <div style={{ padding: "8px 10px", background: "rgba(167,139,250,0.06)", border: "1px solid rgba(167,139,250,0.2)", borderRadius: 5 }}>
              <div style={{ display: "flex", gap: 1, marginBottom: 5 }}>
                {[1,2,3,4,5].map((i) => <span key={i} style={{ fontSize: 12 }}>{i <= 4 ? "★" : "☆"}</span>)}
                <span style={{ fontSize: 10, color: C.muted, marginLeft: 4 }}>4/5 · Conviction Forte</span>
              </div>
              <p style={{ margin: "0 0 6px", fontSize: 10.5, color: C.dim, lineHeight: 1.5 }}>{ANALYST_NOTE}</p>
              <div style={{ fontSize: 9.5, color: C.muted }}>Moussa Ouédraogo · BFD Equity Research · 08 Avr 2026</div>
            </div>
          </div>
          ...
          <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: C.dim, textTransform: "uppercase", letterSpacing: "0.06em" }}>Signaux de Risque</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {RISK_SIGNALS.map((r) => (
                <div key={r.label} style={{ padding: "5px 8px", borderRadius: 4, background: r.color + "08", border: `1px solid ${r.color}22` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 2 }}>
                    ...
                    <span style={{ marginLeft: "auto", fontSize: 9, padding: "0 4px", borderRadius: 2, background: r.color + "18", color: r.color, fontWeight: 800 }}>{r.level}</span>
                  </div>
                  <div style={{ fontSize: 9.5, color: C.muted, lineHeight: 1.4 }}>{r.detail}</div>
                </div>
              ))}
            </div>
          </div>
